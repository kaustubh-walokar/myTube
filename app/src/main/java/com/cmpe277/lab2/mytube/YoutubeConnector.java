...
public class YoutubeConnector {
    ...
    public YoutubeConnector(Context context) {
        session = new Sessions(context);
        accessToken = session.getToken();
        Log.d(Values.TAG, "Token:" + accessToken);
        GoogleCredential credential = new GoogleCredential().setAccessToken(accessToken);
        youtube = new YouTube.Builder(new NetHttpTransport(), new JacksonFactory(), credential).setApplicationName(Values.APPLICATION_NAME).build();
    }

    public List<VideoItem> search(String keywords) {
        try {
            query = youtube.search().list("id,snippet");
            query.setKey(Values.YOUTUBE_PLAYER_KEY);
            query.setType("video");
            query.setMaxResults(Values.NUMBER_OF_VIDEOS_RETURNED);
            ...
            Log.e(Values.TAG, "Error in Video Search: ", e);
    ...
    public String getPlayListDetail() throws Exception {
        ...
        if (channelDetail.getItems().size() > 0) {
            for (Playlist playList : channelDetail.getItems()) {
                if (playList.getSnippet().getTitle().equalsIgnoreCase(Values.PLAYLIST_NAME)) {
    ...
    public String createPlayList() throws Exception {
        ...
        playListObj.setTitle(Values.PLAYLIST_NAME);
        ...
        Log.d(Values.TAG, "Id:" + insertedPlayList.getId());
